...
const LostAndFoundView = ({ user, items, refreshData }) => {
  ...
  const handleSubmit = async () => {
    ...
    await api.postLostAndFoundItem(user.id, {
      itemType,
      description,
      location,
    });
    ...
    >
      ...
      >
        <div style={{ display: "flex", alignItems: "center", gap: "1rem" }}>
          ...
          <div>
            ...
            >
              {items?.length || 0} item{items?.length !== 1 ? "s" : ""} posted
              campus-wide
            </div>
          </div>
        </div>
            ...
            >
              ...
              >
                <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                  <span
                    className={`hms-pill ${i.itemType === "lost" ? "red" : "green"}`}
                  >
                    {i.itemType.toUpperCase()}
                  </span>
                  <span
                    style={{
                      fontSize: 13,
                ...
                <span
                  className={`hms-pill ${i.status === LostAndFoundStatus.OPEN ? "green" : "gray"}`}
                >
                  {i.status}
                </span>
              </div>
              ...
              >
                ...
                >
                  ...
                  <div style={{ fontSize: 11, color: "var(--muted)" }}>
                    Posted by {i.studentName} ·{" "}
                    {new Date(i.datePosted).toLocaleDateString()}
                  </div>
                </div>
                {i.status === LostAndFoundStatus.OPEN &&
                  i.studentId !== user.id && (
                    <button
      ...
      >
        ...
        <div className="hms-form-row">
          ...
          <input
            className="hms-input"
            placeholder="Last seen / Found at (e.g. Library, Mess Hall)…"
            value={location}
