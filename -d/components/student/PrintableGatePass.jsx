  ...
  return (
    <div className="p-8 bg-white text-black font-sans">
      <header className="text-center border-b-2 border-black pb-4 mb-6">
        ...
        >
          Hostel Gate Pass — Official Copy
        </h2>
      </header>
      ...
      <div className="grid grid-cols-3 gap-6">
        <div className="col-span-2 space-y-4">
          <div className="flex items-baseline">
            ...
            <p className="w-2/3 border-b border-dotted border-black">
              {user.name}
            </p>
          </div>
          <div className="flex items-baseline">
          ...
          <div className="flex items-baseline">
            ...
            <p className="w-2/3 border-b border-dotted border-black">
              {room ? `${room.block}-${room.roomNumber}` : "N/A"}
            </p>
          </div>
          <div className="flex items-baseline">
            <p className="w-1/3 font-semibold">Valid From:</p>
            <p className="w-2/3 border-b border-dotted border-black">
              {new Date(pass.fromDate).toLocaleString()}
            </p>
          </div>
          <div className="flex items-baseline">
            <p className="w-1/3 font-semibold">Valid To:</p>
            <p className="w-2/3 border-b border-dotted border-black">
              {new Date(pass.toDate).toLocaleString()}
            </p>
          </div>
          <div className="flex">
            <p className="w-1/3 font-semibold">Reason:</p>
            <p className="w-2/3 h-12 border-b border-dotted border-black pt-1">
              {pass.reason}
            </p>
          </div>
        </div>
      ...
      <footer className="mt-16 flex justify-between items-end">
        <div className="text-center">
          <p className="mt-2 border-t-2 border-dotted border-black pt-1 px-8">
            Student's Signature
          </p>
        </div>
        <div className="text-center">
