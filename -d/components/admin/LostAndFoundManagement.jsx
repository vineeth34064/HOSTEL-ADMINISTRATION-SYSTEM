...
const LostAndFoundManagement = ({ items, refreshData }) => {
    ...
    <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }}>
      ...
      >
        ...
        <div>
          ...
          >
            {items.filter((i) => i.status === LostAndFoundStatus.OPEN).length}{" "}
            open · {items.length} total items
          </div>
        </div>
        <div
          style={{
      ...
      <div className="hms-card">
        <div style={{ padding: 0 }}>
          {!items || items.length === 0 ? (
            ...
            >
              No items on the Lost &amp; Found board.
            </div>
          ) : (
              ...
              <tbody>
                {items.map((item) => (
                  <tr key={item.id}>
                    <td>
                      <span
                        className={`hms-pill ${item.itemType === "lost" ? "red" : "green"}`}
                      >
                    ...
                    <td>
                      <span
                        className={`hms-pill ${item.status === LostAndFoundStatus.OPEN ? "green" : "gray"}`}
                      >
                        {item.status}
                      </span>
                    </td>
