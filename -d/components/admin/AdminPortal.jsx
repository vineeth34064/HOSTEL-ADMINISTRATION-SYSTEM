...
const AdminPortal = ({ user, onLogout, isDarkMode, setIsDarkMode }) => {
  ...
  const renderView = () => {
    if (!adminData)
      return (
        <div className="h-[60vh] flex items-center justify-center dark:text-white text-lg font-bold animate-pulse">
          Initializing Systems...
        </div>
      );
    switch (activeView) {
      case "profile":
        return <ProfilePage user={user} isAdmin />;
      case "health":
        return <AdminHealthRecords />;
      case "gatepass":
        return (
          <GatePassManagement
            requests={adminData.gatePassRequests}
      ...
      case "leave":
        return (
          <LeaveManagement
            requests={adminData.leaveRequests}
            refreshData={fetchData}
          />
        );
      case "complaints":
      ...
      case "fees":
        return (
          <FeeStatus students={adminData.students} refreshData={fetchData} />
        );
      case "mess":
        return (
          <MessMenuEditor menu={adminData.messMenu} refreshData={fetchData} />
        );
      case "parcels":
        return (
          <ParcelManagement
      ...
      case "emergency":
        return (
          <EmergencyManagement
            alerts={adminData.emergencyAlerts}
            refreshData={fetchData}
          />
        );
      case "lost-found":
        return (
          <LostAndFoundManagement
            items={adminData.lostAndFoundItems}
            refreshData={fetchData}
          />
        );
      case "notifications":
      ...
      case "sports":
        return <SportsView user={user} isAdmin />;
      case "antiragging":
        return <AntiRaggingView user={user} />;
      default:
        return <AdminDashboard analytics={adminData.analytics} />;
    }
  };
  const navItems = [
