...
const AdminDashboard: React.FC<{ analytics: AdminDashboardAnalytics | undefined }> = ({ analytics }) => {
    ...
    const statCards = [
        { label: 'Total Students',      value: analytics.totalStudents,     color: 'blue',  Icon: UserIcon         },
        { label: 'On Leave',            value: analytics.studentsOnLeave,   color: 'amber', Icon: FileTextIcon     },
        { label: 'Outside Campus',      value: analytics.studentsOut,       color: 'gold',  Icon: PackageIcon      },
        { label: 'Pending Complaints',  value: analytics.pendingComplaints, color: 'red',   Icon: MessageSquareIcon},
        { label: 'Parcels to Collect',  value: analytics.parcelsToCollect,  color: 'green', Icon: PackageIcon      },
        { label: 'Fees Pending',        value: analytics.feesPending,       color: 'amber', Icon: CreditCardIcon   },
    ];
    ...
    return (
        <motion.div initial="hidden" animate="visible" variants={container} style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
            {/* Header Banner */}
            <motion.div variants={item} style={{
                background: 'linear-gradient(135deg, var(--gold), var(--gold-l))',
                borderRadius: 20, padding: '1.75rem 2rem', marginBottom: '0.5rem',
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
                    ...
                    <div>
                        <div style={{ fontSize: 22, fontWeight: 700, color: '#fff', fontFamily: 'Playfair Display, serif', letterSpacing: '-0.01em' }}>System Overview</div>
                        <div style={{ fontSize: 13, color: 'rgba(255,255,255,0.85)', marginTop: 3 }}>Command Center & Analytics Dashboard</div>
                    </div>
                </div>
                {/* Decorative circles */}
                ...
                <motion.div variants={item} style={{
                    ...
                    <div>
                        <div style={{ fontWeight: 600, color: 'var(--red)', fontSize: 13 }}>
                            {analytics.pendingEmergencies} Active Emergency{analytics.pendingEmergencies > 1 ? 'ies' : 'y'}
                        </div>
                        <div style={{ fontSize: 11, color: 'var(--muted)', marginTop: 2 }}>Go to Emergency section to review and acknowledge.</div>
                    </div>
                </motion.div>
            )}

            {/* Stat cards */}
            <motion.div variants={item} style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '1rem' }}>
                {statCards.map(({ label, value, color, Icon }) => (
                    <div key={label} className={`hms-stat ${color}`} style={{ display: 'flex', alignItems: 'flex-start', gap: 12 }}>
                        <div style={{ flex: 1 }}>
                            <div className="hms-stat-label">{label}</div>
                            <div className="hms-stat-value">{value}</div>
                        </div>
                        <Icon style={{ width: 18, height: 18, color: 'var(--muted)', opacity: 0.5, marginTop: 2, flexShrink: 0 }} />
                    </div>
                ))}
            </motion.div>

            {/* Charts */}
            <motion.div variants={item} style={{ display: 'grid', gridTemplateColumns: '2fr 1fr', gap: '1rem' }}>
                {/* Line chart */}
                ...
                <div className="hms-card">
                    ...
                    <div className="hms-card-body">
                        ...
                        <div style={{ display: 'flex', justifyContent: 'center', gap: 16 }}>
                            {[['var(--green)', `Paid (${analytics.feesPaid})`], ['var(--red)', `Pending (${analytics.feesPending})`]].map(([clr, lbl]) => (
                                <div key={lbl as string} style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 11, color: 'var(--muted)' }}>
                                    <span style={{ width: 8, height: 8, borderRadius: 2, background: clr as string, display: 'inline-block' }} />
                                    {lbl}
                                </div>
                            ))}
                        </div>
