...
interface SportsViewProps { user: User; isAdmin?: boolean; }

const SportsView: React.FC<SportsViewProps> = ({ user, isAdmin }) => {
    ...
    const load = async () => {
        try {
            ...
            setNotifications(all.filter((n: any) => n.category === 'sports'));
        } catch {} finally { setIsLoading(false); }
    };
    useEffect(() => { load(); }, []);

    const handlePost = async () => {
        if (!title || !message) { alert('Please enter a headline and details.'); return; }
    ...
    return (
        <motion.div initial="hidden" animate="visible" variants={container} style={{ maxWidth: 1000 }}>
            {/* Hero Banner */}
            <motion.div variants={item} style={{
                ...
                <motion.div animate={{ rotate: 360 }} transition={{ duration: 20, repeat: Infinity, ease: 'linear' }}
                    style={{ position: 'absolute', right: -50, top: -50, width: 200, height: 200, borderRadius: '50%', background: 'rgba(255,255,255,0.07)', pointerEvents: 'none' }} />
                <motion.div animate={{ x: [0, 40, 0], y: [0, 20, 0] }} transition={{ duration: 8, repeat: Infinity, ease: 'easeInOut' }}
                    style={{ position: 'absolute', left: '30%', bottom: -20, width: 120, height: 120, borderRadius: '50%', background: 'rgba(99,102,241,0.25)', filter: 'blur(24px)', pointerEvents: 'none' }} />
                ...
                <div style={{ position: 'relative', zIndex: 1, display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '1.5rem', flexWrap: 'wrap' }}>
                    ...
                    <motion.div animate={{ y: [0, -12, 0], rotate: [0, 5, 0] }} transition={{ duration: 4, repeat: Infinity, ease: 'easeInOut' }}
                        style={{ padding: '1.25rem', background: 'rgba(255,255,255,0.1)', borderRadius: 24, border: '1px solid rgba(255,255,255,0.2)', backdropFilter: 'blur(10px)', flexShrink: 0 }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: isAdmin ? '280px 1fr' : '1fr', gap: '1.25rem' }}>
                ...
                <div>
                    {isLoading ? (
                        <div style={{ textAlign: 'center', padding: '3rem', color: 'var(--muted)', fontSize: 13 }}>Loading…</div>
                    ) : notifications.length > 0 ? (
                        <AnimatePresence mode="popLayout">
                            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(260px, 1fr))', gap: '1rem' }}>
                                {notifications.map((n, idx) => (
                                    <motion.div key={n.id || idx} layout initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: idx * 0.07, ease: [0.16, 1, 0.3, 1] }}
                                        style={{ background: 'var(--bg)', border: '1px solid var(--border)', borderRadius: 14, overflow: 'hidden', display: 'flex', flexDirection: 'column', transition: 'border-color 0.2s, box-shadow 0.2s' }}
                                        onMouseEnter={e => { (e.currentTarget as HTMLDivElement).style.borderColor = 'var(--gold)'; (e.currentTarget as HTMLDivElement).style.boxShadow = '0 6px 20px rgba(184,149,46,0.15)'; }}
                                            ...
                                            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', paddingTop: '0.75rem', borderTop: '1px solid var(--border)', marginTop: 'auto' }}>
                                                ...
                                                {n.link && (
                                                    <a href={n.link} target="_blank" rel="noopener noreferrer"
                                                        style={{ display: 'inline-flex', alignItems: 'center', gap: 4, fontSize: 10, fontWeight: 700, color: '#fff', background: '#eab308', padding: '4px 10px', borderRadius: 20, textDecoration: 'none' }}>
                        ...
                        <motion.div variants={item} style={{ textAlign: 'center', padding: '4rem 1rem', background: 'var(--bg)', border: '2px dashed var(--border)', borderRadius: 14 }}>
