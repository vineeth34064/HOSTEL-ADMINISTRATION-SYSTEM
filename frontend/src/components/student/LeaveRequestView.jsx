...
const statusMap = { approved: 'green', rejected: 'red', pending: 'gold' };
const LeaveRequestView = ({ user, leaves, refreshData }) => {
    ...
    return (<motion.div initial="hidden" animate="visible" variants={container} style={{ maxWidth: 780 }}>
            ...
            position: 'relative', overflow: 'hidden',
                ...
                <button className="hms-btn" onClick={() => setIsModalOpen(true)} style={{ background: 'rgba(255,255,255,0.2)', color: '#fff', border: '1px solid rgba(255,255,255,0.3)', backdropFilter: 'blur(8px)' }}>
                    + New Request
                </button>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
                {leaves && leaves.length > 0 ? leaves.map((leave) => (<motion.div key={leave.id} variants={item} style={{
                ...
                borderRadius: 14, overflow: 'hidden',
                        ...
                        <div style={{ padding: '1rem 1.25rem', display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
                            ...
                            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0.75rem', marginTop: '0.5rem' }}>
                                {[['From', leave.fromDate], ['To', leave.toDate]].map(([label, date]) => (<div key={label} style={{ background: 'var(--bg2)', borderRadius: 8, padding: '0.625rem 0.875rem' }}>
                                        ...
                                        <div style={{ fontSize: 13, fontWeight: 500, color: 'var(--text)' }}>{date ? new Date(date).toLocaleDateString() : 'Not set'}</div>
                                    </div>))}
                            </div>
                            {leave.createdAt && (<div style={{ fontSize: 11, color: 'var(--muted)', marginTop: 4 }}>
