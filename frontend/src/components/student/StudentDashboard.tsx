...
const StudentDashboard: React.FC<{ user: User; notifications?: Notification[] }> = ({ user, notifications }) => {
        ...
        <motion.div initial="hidden" animate="visible" variants={container} style={{ maxWidth: 820 }}>
            ...
            <motion.div variants={item} style={{
                ...
                <div style={{ textAlign: 'right', flexShrink: 0 }}>
                    ...
                    {user.rollNumber && (
                        <div style={{ fontSize: 12, color: 'rgba(255,255,255,0.7)', marginTop: 6, fontFamily: 'monospace' }}>{user.rollNumber}</div>
                    )}
                </div>
            ...
            <motion.div variants={item} style={{ background: 'var(--bg)', border: '1px solid var(--border)', borderRadius: 14, overflow: 'hidden' }}>
                ...
                {notifications && notifications.length > 0 ? (
                    notifications.map((n, idx) => (
                        <motion.div key={n.id || idx} initial={{ opacity: 0, x: -8 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: idx * 0.05 }}
                            style={{
                            ...
                            {n.createdAt && (
                                <div style={{ fontSize: 10, color: 'var(--muted)', marginTop: 6 }}>{new Date(n.createdAt).toLocaleString()}</div>
                            )}
                        </motion.div>
                    ))
                ) : (
