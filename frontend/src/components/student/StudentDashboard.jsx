...
const StudentDashboard = ({ user, notifications }) => {
    ...
    return (<motion.div initial="hidden" animate="visible" variants={container} style={{ maxWidth: 820 }}>
            {/* Welcome Banner */}
            ...
            position: 'relative', overflow: 'hidden',
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
                    <div style={{ width: 58, height: 58, borderRadius: 16, background: 'rgba(255,255,255,0.25)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 26, fontWeight: 900, color: '#fff', flexShrink: 0 }}>
                        {user.name[0]}
                    </div>
                    <div>
                ...
                <div style={{ textAlign: 'right', flexShrink: 0 }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 6, justifyContent: 'flex-end' }}>
                        ...
                        <span style={{ fontSize: 11, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.12em', color: 'rgba(255,255,255,0.9)' }}>All Systems Operational</span>
                    </div>
                    {user.rollNumber && (<div style={{ fontSize: 12, color: 'rgba(255,255,255,0.7)', marginTop: 6, fontFamily: 'monospace' }}>{user.rollNumber}</div>)}
                </div>
            ...
            {/* Quick stats row */}
            <motion.div variants={item} style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '0.875rem', marginBottom: '1.5rem' }}>
                {[
            { label: 'Fee Status', value: user.feePaid ? 'Cleared' : 'Pending', color: user.feePaid ? 'var(--green)' : 'var(--red)', pill: user.feePaid ? 'green' : 'red' },
            { label: 'Notifications', value: notifications?.length || 0, color: 'var(--accent)', pill: 'blue' },
            { label: 'Access Level', value: user.role?.toUpperCase() || 'STUDENT', color: 'var(--gold)', pill: 'gold' },
        ].map(({ label, value, color, pill }) => (<div key={label} style={{ background: 'var(--bg)', border: '1px solid var(--border)', borderRadius: 14, padding: '1.1rem 1.25rem', position: 'relative', overflow: 'hidden' }}>
            ...
            <motion.div variants={item} style={{ background: 'var(--bg)', border: '1px solid var(--border)', borderRadius: 14, overflow: 'hidden' }}>
                ...
                {notifications && notifications.length > 0 ? (notifications.map((n, idx) => (<motion.div key={n.id || idx} initial={{ opacity: 0, x: -8 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: idx * 0.05 }} style={{
                ...
                borderLeft: '2px solid var(--gold)',
                        ...
                        </motion.div>))) : (<div style={{ padding: '2.5rem', textAlign: 'center', color: 'var(--muted)', fontSize: 13 }}>
