...
export default function HealthRecordView({ user }) {
    ...
    useEffect(() => {
        api.getMyHealthRecord()
            .then(setRecord)
            .catch((err) => setError(err.message || 'Failed to load health record.'))
            .finally(() => setIsLoading(false));
    }, []);
    if (isLoading)
        return <div style={{ padding: '2rem', color: 'var(--muted)', fontSize: 13 }}>Loading Health Record…</div>;
    if (error)
        return <div style={{ padding: '2rem', color: 'var(--red)', fontSize: 13 }}>{error}</div>;
    if (!record)
        return <div style={{ padding: '2rem', color: 'var(--muted)', fontSize: 13 }}>No health record found.</div>;
    ...
    const InfoRow = ({ label, value }) => (<div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '0.5rem 0', borderBottom: '1px solid var(--border)' }}>
    ...
    return (<motion.div initial="hidden" animate="visible" variants={container} style={{ maxWidth: 900 }}>
            {/* Banner */}
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                    ...
                    <motion.div variants={item} style={{ background: 'var(--bg)', border: '1px solid var(--border)', borderRadius: 14, overflow: 'hidden' }}>
                        ...
                        <div style={{ padding: '1rem', display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0.875rem' }}>
                            ...
                            <div style={{ background: 'var(--bg2)', borderRadius: 10, padding: '0.875rem' }}>
                                ...
                                {record.currentMedications.length > 0 ? (<ul style={{ paddingLeft: '1rem', margin: 0 }}>
                    ...
                    <motion.div variants={item} style={{ background: 'var(--bg)', border: '1px solid var(--border)', borderRadius: 14, overflow: 'hidden' }}>
                        ...
                        <div style={{ padding: '1rem', display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
                            {record.existingMedicalConditions.length > 0 && (<div style={{ background: 'rgba(201,64,64,0.07)', border: '1px solid rgba(201,64,64,0.2)', borderRadius: 8, padding: '0.75rem' }}>
            ...
            { label: 'Health Insurance', lines: [record.healthInsuranceDetails.providerName, record.healthInsuranceDetails.policyNumber] },
        ].map(({ label, lines }) => (<div key={label} style={{ background: 'var(--bg)', border: '1px solid var(--border)', borderRadius: 14, overflow: 'hidden' }}>
                                ...
                                <div style={{ padding: '0.875rem' }}>
                                    {lines.map((l, i) => (<div key={i} style={{ fontSize: i === 0 ? 13 : 12, fontWeight: i === 0 ? 600 : 400, color: i === 0 ? 'var(--text)' : 'var(--muted)', marginBottom: 3, fontFamily: i === 1 && label === 'Health Insurance' ? 'monospace' : 'inherit' }}>{l}</div>))}
                                </div>
                            </div>))}
                    </motion.div>
                </div>
