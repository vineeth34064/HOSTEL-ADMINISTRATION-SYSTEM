...
const LostAndFoundView = ({ user, items, refreshData }) => {
    ...
    const filteredItems = useMemo(() => activeTab === 'all' ? items : items?.filter(i => i.itemType === activeTab), [items, activeTab]);
    ...
    return (<motion.div initial="hidden" animate="visible" variants={container} style={{ maxWidth: 780 }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
                {filteredItems && filteredItems.length > 0 ? filteredItems.map(i => (<motion.div key={i.id} variants={item} style={{
                ...
                borderLeft: `4px solid ${i.itemType === 'lost' ? 'var(--red)' : 'var(--green)'}`,
                        ...
                        <div style={{ padding: '1rem 1.25rem', display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: '1rem' }}>
                            ...
                            {i.status === LostAndFoundStatus.OPEN && i.studentId !== user.id && (<button className="hms-btn hms-btn-primary" style={{ flexShrink: 0, fontSize: 12 }} onClick={() => handleClaim(i.id)}>
                                    {i.itemType === 'lost' ? 'I Found This!' : 'This is Mine!'}
                                </button>)}
                        </div>
                    </motion.div>)) : (<motion.div variants={item} style={{ textAlign: 'center', padding: '3rem 1rem', background: 'var(--bg)', border: '2px dashed var(--border)', borderRadius: 14, color: 'var(--muted)', fontSize: 13 }}>
            ...
            <Modal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} title="Post to Lost &amp; Found">
                <div className="hms-form-row">
                    ...
                    <select className="hms-select" value={itemType} onChange={e => setItemType(e.target.value)}>
