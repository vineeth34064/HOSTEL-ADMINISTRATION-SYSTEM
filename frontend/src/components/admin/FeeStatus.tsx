    ...
    return (
        <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
            ...
            <div className="hms-card">
                ...
                <div style={{ padding: 0 }}>
                    {students.length === 0 ? (
                        <div style={{ padding: '2.5rem', textAlign: 'center', color: 'var(--muted)', fontSize: 13 }}>No students found.</div>
                    ) : (
                            ...
                            <tbody>
                                {students.map(s => (
                                    <tr key={s.id}>
                                        ...
                                        <td>
                                            {!s.feePaid && (
                                                <button className="hms-btn hms-btn-sm hms-btn-primary" onClick={() => handleFeeUpdate(s.id)}>Mark Paid</button>
                                            )}
                                        </td>
                                    </tr>
                                ))}
