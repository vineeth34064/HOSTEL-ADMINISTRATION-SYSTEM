...
const AdminNotifications: React.FC = () => {
    ...
    return (
        <motion.div initial="hidden" animate="visible" variants={container} style={{ maxWidth: 860, display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
            {/* Banner */}
                ...
                <div className="hms-card-body" style={{ display: 'flex', flexDirection: 'column', gap: '0.875rem' }}>
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0.875rem' }}>
                        ...
                        <div className="hms-form-row">
                            ...
                            <select className="hms-select" value={category} onChange={e => setCategory(e.target.value as any)}>
            ...
            <motion.div variants={item}>
                ...
                <AnimatePresence mode="popLayout">
                    {notifications.length === 0 ? (
                        <div style={{ textAlign: 'center', padding: '3rem', background: 'var(--bg)', border: '2px dashed var(--border)', borderRadius: 14, color: 'var(--muted)', fontSize: 13 }}>
                            No announcements published yet.
                        </div>
                    ) : (
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
                            {notifications.map((n, idx) => (
                                <motion.div key={n.id || idx} layout initial={{ opacity: 0, x: -16 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: idx * 0.04 }}
                                    style={{
