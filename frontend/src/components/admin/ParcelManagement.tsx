...
const ParcelManagement: React.FC<{ parcels: Parcel[]; students: User[]; refreshData: () => void }> = ({ parcels, students, refreshData }) => {
    ...
    const handleSubmit = async () => {
        if (!studentId || !courier) { alert('Please select a student and enter courier name.'); return; }
        await api.addParcel({ studentId, courier, trackingId });
        refreshData();
        setIsModalOpen(false);
        setStudentId(''); setCourier(''); setTrackingId('');
    };
    ...
    return (
        <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }}>
            ...
            <div className="hms-card">
                <div style={{ padding: 0 }}>
                    {parcels.length === 0 ? (
                        <div style={{ padding: '2.5rem', textAlign: 'center', color: 'var(--muted)', fontSize: 13 }}>No parcels recorded.</div>
                    ) : (
                            ...
                            <tbody>
                                {parcels.map(p => (
                                    <tr key={p.id}>
                                        ...
                                        <td>
                                            {!p.pickedUp && !p.collected && (
                                                <button className="hms-btn hms-btn-sm hms-btn-primary" onClick={() => api.markParcelPickedUp(p.id).then(refreshData)}>
                                                    Mark Picked Up
                                                </button>
                                            )}
                                        </td>
