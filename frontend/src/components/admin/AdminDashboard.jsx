...
const AdminDashboard = ({ analytics }) => {
    if (!analytics)
        return (<div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', height: '60vh', color: 'var(--muted)', fontSize: 14 }}>
            Loading analytics…
        </div>);
    const feePieData = [
    ...
    return (<motion.div initial="hidden" animate="visible" variants={container} style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
            {/* Header Banner */}
            ...
            {/* Emergency banner */}
            {analytics.pendingEmergencies > 0 && (<motion.div variants={item} style={{
            ...
            <motion.div variants={item} style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '1rem' }}>
                {statCards.map(({ label, value, color, Icon }) => (<div key={label} className={`hms-stat ${color}`} style={{ display: 'flex', alignItems: 'flex-start', gap: 12 }}>
                        <div style={{ flex: 1 }}>
