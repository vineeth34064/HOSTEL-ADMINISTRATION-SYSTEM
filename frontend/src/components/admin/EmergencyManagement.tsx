...
const EmergencyManagement: React.FC<{ alerts: EmergencyAlert[]; refreshData: () => void }> = ({ alerts, refreshData }) => {
    ...
    const AlertRow = ({ alert }: { alert: EmergencyAlert }) => {
        ...
        return (
            <div style={{
                ...
                {isActive && (
                    <button className="hms-btn hms-btn-sm hms-btn-primary" onClick={() => handleAcknowledge(alert.id)} style={{ flexShrink: 0 }}>
                        Acknowledge
                    </button>
                )}
            </div>
    ...
    return (
        <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
            ...
            {/* Active alert banner */}
            {active.length > 0 && (
                <div style={{ background: 'rgba(201,64,64,0.08)', border: '1px solid rgba(201,64,64,0.25)', borderRadius: 12, padding: '0.875rem 1.25rem', display: 'flex', alignItems: 'center', gap: 10 }}>
            ...
            <div className="hms-card">
                <div className="hms-card-body">
                    {alerts.length === 0 ? (
                        <div style={{ textAlign: 'center', color: 'var(--muted)', fontSize: 13, padding: '1rem 0' }}>
