      ...
      <div className="hms-auth-card">
        <div className="hms-auth-logo">
          ...
          <div className="hms-auth-sub">
            {isForgotPassword ? 'Reset your password' : 'Welcome back — sign in to your account'}
          </div>
        </div>

        {isForgotPassword ? (<form className="hms-auth-form" onSubmit={handleResetSubmit}>
            ...
            <Button type="submit" className="w-full" style={{ justifyContent: 'center' }}>Send Reset Link</Button>
            
            <div className="hms-auth-toggle" style={{ marginTop: '1.5rem' }}>
              Remember your password?{' '}
              <button type="button" onClick={() => { setIsForgotPassword(false); setResetMessage(''); }}>Back to login</button>
            </div>
          </form>) : (<>
            <form className="hms-auth-form" onSubmit={onSubmit}>
              ...
              <div className="hms-form-row">
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '0.4rem' }}>
                  <label className="hms-label" style={{ margin: 0 }}>Password</label>
                  <button type="button" onClick={() => setIsForgotPassword(true)} style={{ background: 'none', border: 'none', fontSize: '11px', color: 'var(--gold)', cursor: 'pointer', padding: 0 }}>Forgot password?</button>
                </div>
