...
const Sidebar = ({ navItems, onLogout, user, isDarkMode, setIsDarkMode }) => {
    const [mobileOpen, setMobileOpen] = useState(false);
    const location = useLocation();

    // Determine path for active styling (strip slash)
    ...
    const SidebarContent = () => (
        <div className="hms-sidebar flex flex-col h-full">
            ...
            <div className="hms-sidebar-logo">
                ...
                {setIsDarkMode && (
                    <button onClick={() => setIsDarkMode(!isDarkMode)} className="hms-dark-toggle ml-auto" aria-label="Toggle dark mode">
            ...
            <div className="hms-sidebar-nav flex-1">
                {navItems.map((item) => (
                    <Link
                        ...
                        className={`hms-nav-item ${activeId === item.id ? 'active' : ''}`}
                    >
    ...
    return (
        <>
            ...
            {/* Mobile overlay */}
            {mobileOpen && (
                <div className="lg:hidden fixed inset-0 bg-black/30 z-30 print:hidden" onClick={() => setMobileOpen(false)} />
            )}
