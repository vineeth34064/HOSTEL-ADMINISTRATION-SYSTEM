...
export default function AIChatbot() {
    ...
    const handleSend = async (e) => {
        ...
        try {
            ...
            if (res.contextData) setContextData(p => ({ ...p, ...res.contextData }));
    ...
    // Calculate centering position purely in JS so Framer Motion cannot override it
    const isMobile = typeof window !== 'undefined' && window.innerWidth < 768;
    const chatWidth = isMobile ? Math.min(window.innerWidth * 0.92, 420) : 360;
    const wrapperStyle = {
        position: 'fixed',
        zIndex: 10000,
        bottom: 90,
        width: chatWidth,
        // Mobile: center horizontally. Desktop: anchor to right edge.
        ...(isMobile
            ? { left: '50%', marginLeft: -(chatWidth / 2) }
            : { right: 24, left: 'auto' }
        ),
    };
    ...
    return (
        <>
            ...
            {/* Chat window — wrapper div centres, motion.div only animates */}
            <AnimatePresence>
                {isOpen && (
                                    ...
                                    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} style={{ display: 'flex', gap: 8, alignItems: 'flex-end' }}>
                                        ...
                                        <div style={{ padding: '0.625rem 1rem', background: 'var(--bg)', border: '1px solid var(--border)', borderRadius: '18px 18px 18px 4px', display: 'flex', gap: 5, alignItems: 'center' }}>
                                            {[0, 0.15, 0.3].map((delay, i) => (
                                                <motion.div key={i} animate={{ y: [0, -5, 0] }} transition={{ duration: 0.7, repeat: Infinity, delay }} style={{ width: 6, height: 6, borderRadius: '50%', background: 'var(--muted)' }}/>
                                            ))}
                                        </div>
                                    </motion.div>
                                )}
                            ...
                            <form onSubmit={handleSend} style={{
                                ...
                                <button
                                    ...
                                    style={{
                                        ...
                                        display: 'flex', alignItems: 'center', justifyContent: 'center',
                                        boxShadow: input.trim() && !isLoading ? '0 4px 12px rgba(184,149,46,0.3)' : 'none',
                                    }}
                                >
                                    <SendIcon />
                                </button>
